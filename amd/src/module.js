            ...
            <div class="list-group">
                ${capabilities.explain ?
                    `<a href="#" class="list-group-item list-group-item-action" data-action="explain">
                        <i class="fa fa-info-circle"></i> ${M.util.get_string('explain', 'aiplacement_textinsights')}
                    </a>` : ''}
                ${capabilities.summarize ?
                    `<a href="#" class="list-group-item list-group-item-action" data-action="summarize">
                        <i class="fa fa-compress"></i> ${M.util.get_string('summarize', 'aiplacement_textinsights')}
                    </a>` : ''}
                ${capabilities.validate ?
                    `<a href="#" class="list-group-item list-group-item-action" data-action="validate">
                        <i class="fa fa-check-circle"></i> ${M.util.get_string('validate', 'aiplacement_textinsights')}
                    </a>` : ''}
            </div>
            <div class="ai-icon-bottom-right">
        <div class="ai-icon-bottom-right"> ${M.util.get_string('poweredby', 'aiplacement_textinsights')}
    ...
    const processText = async (text, action) => {
        try {
            const result = await fetchMany([{
                methodname: 'aiplacement_textinsights_process_text',
                args: {
                    text: text,
                    action: action,
                    courseid: courseId
            ...
            return M.util.get_string('error', 'aiplacement_textinsights');
        }
    };
    ...
    $menu.on('click', '[data-action]', async function(e) {
        ...
        $tooltip.find('.textinsights-tooltip-content')
            .html('<div class="textinsights-loading"><i class="fa fa-loading fa-spin"></i> ' +
                  M.util.get_string('loading', 'aiplacement_textinsights') + '</div>');
        $tooltip.show();
